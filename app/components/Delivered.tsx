...
function Entry({ p, index }: { p: (typeof siteConfig.projects)[number]; index: number }) {
  ...
  return (
    <li ref={ref} className="g-rule-b grid grid-cols-1 gap-y-5 py-6 md:grid-cols-[76px_1fr_1.5fr_auto] md:items-start md:gap-x-8 md:py-7">
      <div className="rv-ink" style={{ '--i': index * 3 } as React.CSSProperties}>
        <span className="g-field-label md:mb-0">联次</span>
        <span className="font-mono text-[20px] text-on-ground">
          {String(index + 1).padStart(2, '0')}
        </span>
      </div>

      <div className="rv-ink" style={{ '--i': index * 3 + 1 } as React.CSSProperties}>
        <span className="g-field-label">项目名称</span>
        <h3 className="font-display text-[clamp(1.05rem,2vw,1.3rem)] leading-snug text-on-ground">
          {p.name}
        </h3>
        <p className="mt-1 font-body text-[13px] text-on-muted">{p.type}</p>
        <ul className="mt-3 flex flex-wrap gap-x-3 gap-y-1">
          {p.tech.map((t) => (
            <li key={t} className="font-mono text-[11px] text-on-muted">
              {t}
            </li>
          ))}
        </ul>
      ...
      <div className="rv-ink" style={{ '--i': index * 3 + 2 } as React.CSSProperties}>
        <span className="g-field-label">交付内容</span>
        <p className="max-w-[54ch] text-[14px] leading-relaxed text-on-muted">{p.description}</p>
      </div>
      ...
      >
        ...
        <figure className="flex flex-col items-start gap-2 md:items-end">
          <img
            ...
            loading="lazy"
            decoding="async"
            className="max-h-[156px] w-auto max-w-full border border-on-ground/30 md:max-h-[176px] md:max-w-[300px]"
          />
          <figcaption className="font-body text-[11px] text-on-muted md:mt-1.5">
            附件 · 实机界面
          </figcaption>
        </figure>

        <span className="g-stamp-mark text-[12px]" data-stamped={stamped}>
          已交付
        </span>
      </div>
    </li>

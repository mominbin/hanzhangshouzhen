  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-y-10 py-8 md:grid-cols-[1fr_1.3fr] md:gap-x-12 md:py-10">
        <div className="rv-ink border-b border-rule pb-8 md:border-b-0 md:pb-0">
        ...
        <div>
          ...
          <ul>
            {remarks.map((r, i) => (
              <li
                key={r.title}
                className="rv-ink rule-b grid grid-cols-[40px_1fr] items-baseline gap-x-4 py-4 transition-colors duration-150 hover:bg-hover"
      ...
      <div className="rule-t rule-b py-6">
        <span className="field-label rv-ink" style={{ '--i': 5 } as React.CSSProperties}>
          背书单位 / ENDORSED BY
        </span>
        <ul className="mt-2 flex flex-wrap items-center gap-x-10 gap-y-3">
          {siteConfig.partners.map((p, i) => (
            <li
              key={p.name}
              className="rv-ink font-display text-[15px] tracking-[0.02em] text-ink-muted"
        ...
        <p className="rv-ink mt-3 max-w-[70ch] text-[12px] text-ink-muted" style={{ '--i': 10 } as React.CSSProperties}>
